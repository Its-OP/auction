...
const ChangeContext = createContext<lotForEditType>({
  lotForEdit: null,
  setLotForEdit: () => {},
});
export const ChangeLotContextProvider: FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [lotForEdit, setLotForEdit] = useState<Lot | null>(null);

  return (
    <ChangeContext.Provider value={{ lotForEdit, setLotForEdit }}>
      {children}
