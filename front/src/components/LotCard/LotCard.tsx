...
export const LotCard: JSXElementConstructor<{ lot: Lot }> = ({ lot }) => {
  ...
  return (
    <Card
      ...
      extra={<Link to={`/lot/${id}`}>Детальніше</Link>}
    >
      {image ? (
        <Image src={image} width={"100%"} style={{ maxHeight: 300 }} />
      ) : (
        <Empty />
      )}
      ...
      <div style={{ padding: "0 24px 24px" }}>
        ...
        >
          <Typography.Title level={5}>
            Стартова ціна: {currencyFormat(minPrice)}
          </Typography.Title>
          <Typography.Text style={{ marginTop: 20 }} type={"secondary"}>
            № лоту: {id}
          </Typography.Text>
        </div>
