...
public class AuctionsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAuctions(CancellationToken token, [FromQuery] int pageSize = 10, [FromQuery] int pageNumber = 1,
        [FromQuery] string search = "", [FromQuery] string sort = "asc")
    {
        if (pageSize < 1)
        ...
        var auctions = _context.Auctions.Where(x => x.Title.Contains(search));

        if (sort == "asc")
        {
            auctions = auctions.OrderBy(x => x.MinPrice);
        }
        else if (sort == "desc")
        {
            auctions = auctions.OrderByDescending(x => x.MinPrice);
        }
        else
        {
        ...
        var res = await auctions.Select(x => new AuctionContract(x)).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync(token);
